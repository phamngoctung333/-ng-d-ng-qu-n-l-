...
use Illuminate\Support\Facades\Session; // Thêm dòng này để import Session
...
class LoadController extends Controller
{
    ...
    public function search(Request $request)
    {
        $mssv = $request->input('mssv');
            ...
            return view('pages.admin_ttsv',['ttsv'=>$ttsv,'name'=>$name]);
    }
...
#----------Đăng_kí_phòng_ở------------------------------------------------------------------------------------------
    ...
    public function get_student_dkphong($id){
        ...
        $giaphong = $ttphong->gia;
        $mssv = $ttsv->mssv;
        $gtsv = $ttsv->gtsv;
        $sncur = $ttphong->sncur;
        $snmax = $ttphong->snmax;
        $gioitinh = khuktx::where('id',$id_khu)->value('gioitinh');
    ...
    #-----------------------------Admin-------------------------------------------
        ...
        public function post_admin_statics(Request $request) {
            $data = DB::table('sinhvien')
                ->join('phieudangky', 'phieudangky.mssv', '=', 'sinhvien.mssv')
            ...
            if (isset($year) && isset($id_khu)) {
                ...
                // Tính số người đang ở và số chỗ còn trống
                $sncur_total = phong::where('id_khu', $id_khu)->sum('sncur'); // Tổng số người đang ở
                $snmax_total = phong::where('id_khu', $id_khu)->sum('snmax'); // Tổng số chỗ ở
                $empty_space = $snmax_total - $sncur_total; // Số chỗ còn trống
        
                // Đếm số sinh viên đã đăng ký trong năm và khu đã chọn
                $nam_dkcur = 0; // Số sinh viên đang ở (đã đăng ký) trong năm hiện tại
                $nu_dkcur = 0; // Số sinh viên chưa có người đăng ký (có thể bỏ nếu không cần)
        
                foreach ($data as $d) {
                    if (($d->nam == $year) && ($d->id_khu == $id_khu) && ($d->trangthaidk != 'cancelled')) {
                        // Nếu bạn cần đếm theo giới tính, bạn có thể giữ lại
                        if ($d->gtsv == 'nam') {
                            $nam_dkcur++;
                        } else {
                            $nu_dkcur++;
                ...
                // Tổng số sinh viên lưu trú trong năm
                $total_student = DB::table('phieudangky')
                    ->join('phong', 'phieudangky.id_phong', '=', 'phong.id')
                    ->join('khuktx', 'phong.id_khu', '=', 'khuktx.id')
                    ->where('khuktx.id', $id_khu) // Lọc theo khu
                ...
                return view('pages.admin_statics', [
                    'sncur' => $sncur_total, // Tổng số người đang ở
